...
A Managed Deep Agent is a [project directory](/langsmith/managed-deep-agents-overview#your-agent-is-a-directory) with a required agent entry (`agent.py`, `agent.ts`, or `agent.tsx`). To scaffold a project and chat with the agent locally:
...
# Add LANGSMITH_API_KEY and your model provider key to .env
uv sync
mda dev .
```
...
`mda dev` compiles the project and starts a local LangGraph dev server in LangSmith Studio, where you can send the agent a message. The steps below expand each part, then deploy the agent to LangSmith.
...
The managed runtime owns `backend`, `store`, `checkpointer`, `memory`, `skills`, and the system prompt. Do not set those fields in the agent definition. For the full list of author-set and managed fields, see the [agent definition reference](/langsmith/managed-deep-agents-cli#agent-definition-reference).
...
<CardGroup cols={2}>
  <Card title="Tutorial" icon="book" href="/langsmith/managed-deep-agents-tutorial">
    Build a scheduled research agent with tools, memory, and a cron schedule.
  </Card>
