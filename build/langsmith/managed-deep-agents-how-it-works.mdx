...
Managed Deep Agents turns a local [project directory](/langsmith/managed-deep-agents-overview#your-agent-is-a-directory) into a hosted LangGraph deployment. Understanding what the `mda` CLI compiles, what a deploy creates, and which parts the runtime owns helps you reason about behavior, secrets, and state.
...
`mda deploy` validates the project, syncs deploy-owned context to Context Hub, uploads the compiled source, triggers a hosted build, and reconciles cron schedules once the deployment is live.
...
- **Non-reserved `.env` values** (model provider keys, MCP tokens, database URLs) are forwarded as hosted deployment secrets. The `.env` file itself is never archived.
...
When you pass `--no-wait`, the CLI triggers the build and exits before the deployment reaches `DEPLOYED`, so schedule reconciliation is skipped for that invocation. For every command flag and the full step list, see [Deploy an agent](/langsmith/managed-deep-agents-deploy) and the [CLI reference](/langsmith/managed-deep-agents-cli#deploy-projects).

## What the managed runtime owns

The managed runtime owns infrastructure so the agent definition can focus on behavior. Do not set the managed fields in the agent definition:
...
The managed runtime owns the checkpointer and store, so each thread's state persists across runs without any setup. Durable memory lives in Context Hub at `/memories/AGENTS.md` and is available to the agent across threads.
...
- **Lifecycle.** Reuse and lifecycle knobs such as `idle_ttl_seconds` and `default_timeout` control how long a sandbox is retained and how long commands can run.
- **Provisioning.** When `sandbox/setup.sh` exists, MDA runs it once when a new managed sandbox is provisioned. Use it to install packages, seed files, or prepare workspace state.
