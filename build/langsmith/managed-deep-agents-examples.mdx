...
For the packaging rules behind this layout, see [How Managed Deep Agents work](/langsmith/managed-deep-agents-how-it-works#compile).

## Compose the agent

The agent entry imports the authored tool and middleware, then declares the model, middleware order, and which tools pause for human review. The managed runtime injects backend, store, checkpointer, memory, skills, and the system prompt, so none are set here.
...
The connector declares a remote MCP server. MDA loads its tools at runtime and appends them to the authored tools. For more, see [Connect MCP tools](/langsmith/managed-deep-agents-mcp).
