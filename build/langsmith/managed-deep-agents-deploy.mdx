...
This page covers project files, compiled output, local development, Context Hub sync, schedules, sandboxes, and deploy behavior. For command flags, see the [CLI reference](/langsmith/managed-deep-agents-cli).
...
A Managed Deep Agents project starts with an agent entry and optional project folders. For the full file layout and packaging rules, see [the project directory layout](/langsmith/managed-deep-agents-overview#your-agent-is-a-directory).
...
6. Compile the project into `.mda/build`, including optional `connectors/mcp` declarations, MCP adapter dependencies, and `schedules/` declarations.
...
10. Reconcile MDA-owned LangSmith cron jobs for local `schedules/` declarations.
