...
Build and server logs are available for each revision. LangSmith Cloud also supports native Datadog forwarding for server logs and traces. To enable Datadog logs, configure the deployment with `DD_API_KEY`, `DD_LOGS_ENABLED=true`, and `DD_LOG_INJECTION=true`. For more information, see [`DD_API_KEY`](https://docs.langchain.com/langsmith/env-var-cloud#dd_api_key).
